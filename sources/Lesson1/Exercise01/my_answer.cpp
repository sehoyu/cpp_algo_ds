...
template <typename T>
class dynamic_array {
  T* data; // T 타입 포인터 = 배열 이름 = 상수 포인터 = 다른 주소 변경 x
  size_t n;
...
public:
  ...
  dynamic_array(const dynamic_array<T>& other) {
    ...
    // ! c++ 배열은 공간 할당 후에 각 공간에 값을 할당
  }

  // ? 왜 인덱스 검사를 안 할까?
  // * C++의 Zero-overhead 철학: 성능 최적화를 위해(e.g. 반복문) 호출자에게 책임을 위임함. 
  // * 필요하다면 assert()를 사용해 디버그 모드에서만 검사하는 것이 관례.
  ...
  friend dynamic_array<T> operator+(const dynamic_array<T>& arr1,
    ...
    dynamic_array<T> result = new T[arr1.size()+arr2.size()];
